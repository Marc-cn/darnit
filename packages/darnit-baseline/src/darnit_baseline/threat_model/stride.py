...
def calculate_risk_score(
    exposure: str, data_sensitivity: str, has_auth: bool, has_input_validation: bool, existing_controls: list[str]
) -> RiskScore:
    ...
    """
    # Likelihood factors
    exposure_weights = {"public": 1.0, "authenticated": 0.7, "internal": 0.4, "localhost": 0.1}
...
def analyze_stride_threats(assets: AssetInventory, injection_sinks: list[dict[str, Any]]) -> list[Threat]:
    ...
    if pii_fields:
        ...
        threats.append(
            Threat(
                ...
                code_locations=[
                    CodeLocation(
                        file=sd.file, line_start=sd.line, line_end=sd.line, annotation=f"PII field: {sd.field_name}"
                    )
    ...
    if public_endpoints:
        ...
        threats.append(
            Threat(
                ...
                code_locations=[
                    CodeLocation(
                        file=ep.file, line_start=ep.line, line_end=ep.line, annotation=f"Public endpoint: {ep.path}"
                    )
...
def identify_control_gaps(assets: AssetInventory, threats: list[Threat]) -> list[dict[str, Any]]:
