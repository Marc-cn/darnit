...
- **Mixed entry points** (cobra + HTTP routes in the same repository): both surface in the output; neither suppresses the other.
...
- **FR-003**: The generator MUST group sibling subcommands into command families for rendering, so a project with N commands does not produce N individually-rendered findings. Subcommands of the same parent surface as one family with the individual commands listed as locations within it.
...
- **FR-005**: Each finding MUST be assigned at least one STRIDE category. The category SHOULD be inferred from the command's apparent function (argument handling, file I/O, network I/O, signature operations, etc.) where that function is discernible from the source. Where the function is opaque, a placeholder category MAY be used provided the finding is explicitly marked as needing reviewer attention.
...
- **SC-006**: For findings inspected by an independent reviewer, at least 70% of the assigned STRIDE categories are judged "plausible" (consistent with the command's apparent function). The remaining findings may need recategorisation but must not be obvious nonsense.
